...
describe('With a pattern row editor', function () {
  ...
  it('A designer moves a row by dragging the row move handle', function () {
    cy.get('.row-move-handle .pointer-target')
      .as('pointer-target')
      .pointer('down')
    cy.get('.row-move-handle')
      .as('handle')
      .should('have.class', 'is-dragged')
    cy.percySnapshot(`${testTitle()} [at pointerdown]`)
    cy.get('@pointer-target')
      .pointermoveBy({ dX: 100, dY: 200 })
    cy.percySnapshot(`${testTitle()} [at pointermove]`)
    cy.get('@pointer-target')
      .pointer('up')
    cy.get('@handle')
  ...
  it('Dragging of the row move handle is cancelled', function () {
    cy.get('.row-move-handle .pointer-target')
      .as('pointer-target')
      .pointer('down')
      .pointermoveBy({ dX: -25, dY: -25 })
    cy.get('.row-move-handle')
      .as('handle')
      .should('have.class', 'is-dragged')
    cy.percySnapshot(`${testTitle()} [at pointermove]`)
